...
def make_matrix(S, SMOOTH, normalization, shift, eof_interval=None, clim_eof_interval=None, **kwargs):
    '''
    ...
    eof_interval: list, optional
        List with the starting and ending date for EOF calculation.
        If provided, EOF modes are computed only on this period.
        If None, EOF modes are computed from entire time series.
    clim_eof_interval: list, optional
        List with the starting and ending date for computing climatology.
        If provided, anomalies are computed by subtracting climatology computed
        only on this period. If None, climatology is computed from entire time series.
    dropnan: String
    ...
    # Create anomalies
    if clim_eof_interval is not None:
        # Compute climatology only on clim_eof_interval period
        print(f'make_matrix: -- Computing climatology from {clim_eof_interval[0]} to {clim_eof_interval[1]}')
        climstart = X.A.time.to_index().get_loc(clim_eof_interval[0])
        climend = X.A.time.to_index().get_loc(clim_eof_interval[1])
        
        # Extract the period for climatology
        X_clim_period = X.A.isel(time=slice(climstart, climend+1))
        
        # Compute climatology on this period only
        clim = X_clim_period.groupby('time.month').mean('time')
        
        # Apply anomaly using the precomputed climatology
        X.A = X.A.groupby('time.month') - clim
        
        # Normalize by std if needed
        if normalization == 'STD':
            climstd = X_clim_period.groupby('time.month').std('time')
            X.A = X.A.groupby('time.month') / (climstd + 1.0e-14)
    else:
        # Standard anomaly computation using full time series
        print(f'make_matrix: -- Computing climatology from full time series')
...
def make_data_base(InputVars, period='ANN', version='V5', SMOOTH=False, normalization='STD', \
                   eof_interval = None, clim_eof_interval=None, detrend=False, \
    ...
    eof_interval: list
        List with the starting and ending date for EOF analysis
    clim_eof_interval: list
        List with the starting and ending date for computing climatology.
        If provided, anomalies are computed using climatology from this period.
        If None, climatology is computed from entire time series.
    detrend: boolean
        If True, detrend the data
    shift: string
        Type of shift to be applied to select the data
    ...
    for invar in InputVars:
        for inlevel in invar.levels:
            ...
            X = make_matrix(S, SMOOTH, normalization, shift, eof_interval=eof_interval, clim_eof_interval=clim_eof_interval, dropnan=False, detrend=detrend)
